...
class node
{
   ...
   node *prev;
   node *next;
   //implement DLL: create and display
   ...
   node *deleteCenter(node *start);
   node *deleteAft(node *start);
   node *deleteAfCount(node *start);
};
...
node *node::deleteCenter(node *start)
{
    node *ptr;
    int count = 0;
    int i = 0;
    ...
    {
        ptr = start;
        //getting the total no. of elements in the link list
        while(ptr != NULL)
        {
            count ++;
            ptr = ptr -> next;
      ...
      if(count>2)
      {
        ...
        delete(ptr);
        cout<<"Center Node deleted successfully"<<endl;
      }
      else
      {
         cout<<"Only two nodes are there !!!!"<<endl;
      }
    }
...
//delete the node after a certain count of element;
node *node::deleteAfCount(node *start)
{
    int n,count = 0;
    int i = 0;
    node *ptr; 
    cout<<"Enter count[No. after which the node will be deleted]"<<endl;
    cin>> n;
    //getting the total no. of elements in the link list
    ptr = start;
    while(ptr != NULL)
    ...
    if(n==count)
    {
        cout<<"This is the last Element in the list.....Deletion after this node cannot be performed"<<endl;
    }
    else if(n>count)
    {
        cout<<"there are only "<<count<<" nodes in the link list.."<<endl;
    }
    else
    {
        ptr = start;
...
int main()
{
  ...
  while(1)
  {
   ...
   cout<<"9. delete the center node in the Link List"<<endl;
   cout<<"10. delete after the specified node in the Link List"<<endl;
   cout<<"11. delete the node after certain count"<<endl;
   ...
   switch(choice)
   {
      ...
      case 8:
           ...
           break;
      case 9:
            start = acc.deleteCenter(start);
            break;
      case 10:
      	  start = acc.deleteAft(start);
           break;
      case 11:
      	  start = acc.deleteAfCount(start);
           break;
         default:
